...
with st.sidebar:
    ...
    # ✅ Clear Chat: only messages, not vector store / 仅清会话，不动向量库
...
if st.session_state.page == "chat":
    ...
    if uploaded:
        build_disabled = not bool(os.getenv("OPENAI_API_KEY"))  # guard if no key / 无密钥则禁用
        ...
        # ===== Reset vector store + clear chat / 重置知识库并清空聊天 =====
        if reset_clicked:
            # 1) drop vector store & chain / 移除内存中的向量库与链
            ...
            if chain and getattr(chain, "memory", None):
                try:
                    ...
                    pass
            # 2) also clear chat history / 同时清空聊天记录
            clear_chat_history()
            # 3) info banner / 信息提示
            st.info(
